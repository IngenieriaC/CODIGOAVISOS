...
def load_and_merge_data(uploaded_file):
    ...
    if 'costes_tot_reales' in df_avisos.columns:
        ...
        st.warning("La columna 'costes_tot_reales' no se encontró. Los análisis de costos no estarán disponibles.")
    ...
    if 'duracion_de_parada' in df_avisos.columns:
        ...
        st.warning("La columna 'duracion_de_parada' no se encontró. Los cálculos de MTTR, MTBF y Disponibilidad pueden ser incorrectos.")
    ...
    if st.session_state.df is None or st.session_state.df.empty:
        ...
        if not eval_targets:
            ...
            for metric_name, values_dict in technical_metrics_row_data.items():
                row = {"Categoría": "Desempeño técnico", "Pregunta": metric_name}
                for target in matrix_targets:
            ...
            if matrix_data:
                ...
                # Check if there are any actual targets (columns other than 'Categoría', 'Pregunta')
                data_columns = [col for col in evaluation_matrix_df.columns if col not in ["Categoría", "Pregunta"]]

                if not data_columns:
                    st.info("Aún no se han registrado evaluaciones manuales para los objetivos seleccionados. La matriz mostrará solo métricas técnicas si están disponibles.")
                else:
                    ...
                    for target in data_columns:
                        ...
                        score_rows.append({"Categoría": "Total Puntuación", "Pregunta": "", target: total_score})
                        score_rows.append({"Categoría": "Puntuación Promedio", "Pregunta": "", target: f"{avg_score:.2f}"})

                    if score_rows:
                        score_df = pd.DataFrame(score_rows)
                        evaluation_matrix_df = pd.concat([evaluation_matrix_df, score_df], ignore_index=True)
                        st.dataframe(evaluation_matrix_df.set_index(["Categoría", "Pregunta"]))
                    else:

...
def load_and_merge_data(uploaded_file_buffer: io.BytesIO) -> pd.DataFrame:
    ...
    # Asegúrate de que 'Descripción' también esté en el equipo_original si se usa más adelante
    ...
    columnas_finales = [
        ...
        "Texto grupo acción", "TIPO DE SERVICIO", # Asegúrate de que TIPO DE SERVICIO esté aquí
...
def calcular_disponibilidad(df_subset: pd.DataFrame, horarios: dict) -> pd.Series:
    ...
    # Convertir 'Duración de parada' a numérica, reemplazando no numéricos con 0
    df_subset['Duración de parada'] = pd.to_numeric(df_subset['Duración de parada'], errors='coerce').fillna(0)

    # Identificar horarios de los equipos. Asumo que el horario se puede inferir del 'Equipo' o 'Denominación de objeto técnico'
    # Esta parte puede necesitar ajuste si la columna 'HORARIO' no existe o si la lógica es más compleja
    # Para el ejemplo, usaremos 'Denominación de objeto técnico' para buscar el horario
    
    # Crea una columna 'Horario_Asignado' basada en la 'Denominación de objeto técnico' o 'Equipo'
    # Asume que el horario está en 'Texto_equipo' o alguna columna similar que no está en el dataset de avisos directamente
    # Para este ejemplo, lo dejaremos como un valor fijo si no hay una columna que mapee a los horarios_dict keys.
    # Si 'ih08' tiene una columna 'Horario' o similar, se debería usar. Por ahora, asumimos un horario por defecto si no se encuentra.

    # Paso 1: Intentar obtener el 'HORARIO_XX' desde 'Denominación de objeto técnico' o 'Texto_equipo' si es posible
    # Esto es una suposición. Si el 'HORARIO_XX' está en otra columna, se debe ajustar.
    ...
    # Si no se encuentra un horario específico para el equipo, se puede asignar uno por defecto o un promedio
    # Para este ejemplo, usaremos un promedio de horas/día y días/año si no se encuentra un horario específico
    ...
    # Agrupar por tipo de servicio (o equipo, si la evaluación es por equipo) y calcular la disponibilidad
    # La disponibilidad se calcula por equipo, y luego se podría promediar por tipo de servicio si es necesario.
    
    # Calculamos la suma de duración de parada por equipo
    sum_parada_equipo = df_subset.groupby('Equipo')['Duración de parada'].sum()
    
    # Obtenemos las horas operativas totales únicas por equipo
    horas_op_equipo = df_subset.drop_duplicates(subset='Equipo').set_index('Equipo')['Horas_Operativas_Totales']

    # Asegurarse de que ambos Series tienen el mismo índice para la operación
...
def calcular_mttr(df_subset: pd.DataFrame) -> pd.Series:
    ...
    if df_subset.empty:
        return pd.Series(dtype=float)
    df_subset['Duración de parada'] = pd.to_numeric(df_subset['Duración de parada'], errors='coerce').fillna(0)
    # MTTR = Suma de Duración de Parada / Cantidad de Avisos
...
def calcular_mtbf(df_subset: pd.DataFrame, horarios: dict) -> pd.Series:
    ...
    # Calcular horas operativas totales por equipo (reusando la lógica de disponibilidad)
    df_subset['Horario_Key'] = df_subset['Denominación de objeto técnico'].apply(
        lambda x: next((key for key in horarios.keys() if key.lower() in str(x).lower()), None)
    )
    default_horas_dia = np.mean([h[0] for h in horarios.values()])
    ...
    # Número de avisos por equipo (considerando cada aviso como una "falla" para MTBF simplificado)
    ...
    # Asegurarse de que todos los Series tienen el mismo índice
    ...
    # Si la evaluación es por TIPO DE SERVICIO, promediamos el MTBF de los equipos por servicio
    ...
    with st.spinner('Cargando y procesando datos... Esto puede tomar un momento.'):
        try:
            ...
            if 'Denominación ejecutante' in df_processed.columns:
                ...
                st.warning("La columna 'Denominación ejecutante' no se encontró para usar como 'PROVEEDOR'.")

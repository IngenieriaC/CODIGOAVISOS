...
# --- Definition of detailed questions and ranges ---
rangos_detallados = {
    "Calidad": {
...
# --- INDICATOR CALCULATION FUNCTIONS (Modified to calculate by Provider within a Service Type) ---
def calcular_indicadores(df_filtered_data, group_col='PROVEEDOR'):
    """
    Calculates service indicators (MTTR, MTBF, Disp, Rendimiento) grouped by a column.
    Args:
        df_filtered_data (pd.DataFrame): Filtered DataFrame.
        group_col (str): Column to group by (e.g., 'PROVEEDOR' or 'TIPO DE SERVICIO').
    Returns:
        tuple: Pandas Series with the grouped indicators (count, cost, mttr, mtbf, disp, rend).
    """
    ...
    if not all(col in df_filtered_data.columns for col in required_cols):
        missing_cols = set(required_cols) - set(df_filtered_data.columns)
    ...
    # Handle potential division by zero or NaN values in ttot calculation
...
class CostosAvisosApp:
    def __init__(self, df):
        self.df = df
        self.EJECUTANTE_COL_NAME_NORMALIZED = 'PROVEEDOR' # Changed to use the new 'PROVEEDOR' column
        self.COL_COSTOS_NORMALIZED = 'COSTO' # Changed to use the new 'COSTO' column
        self.COL_AVISO_NORMALIZED = 'AVISO' # Changed to use the new 'AVISO' column
        self.COL_FECHA_AVISO_NORMALIZED = 'Fecha de aviso' # Original column name
        ...
        self.opciones_menu = {
            ...
            "Costos por objeto técnico": ("Denominación de objeto técnico", self.COL_COSTOS_NORMALIZED, "costos"),
            "Avisos por objeto técnico": ("Denominación de objeto técnico", None, "avisos"),
            "Costos por texto código acción": ("Texto código acción", self.COL_COSTOS_NORMALIZED, "costos"),
            "Avisos por texto código acción": ("Texto código acción", None, "avisos"),
            "Costos por texto de acción": ("Texto de acción", self.COL_COSTOS_NORMALIZED, "costos"),
            "Avisos por texto de acción": ("Texto de acción", None, "avisos"),
            "Costos por tipo de servicio": ("TIPO DE SERVICIO", self.COL_COSTOS_NORMALIZED, "costos"),
            "Avisos por tipo de servicio": ("TIPO DE SERVICIO", None, "avisos"),
    ...
    def display_costos_avisos_dashboard(self):
        ...
        st.dataframe(filtered_df_costos[[self.COL_AVISO_NORMALIZED, self.COL_FECHA_AVISO_NORMALIZED, 'PROVEEDOR', 'TIPO DE SERVICIO', 'Descripción', self.COL_COSTOS_NORMALIZED, 'TIEMPO PARADA']].head(100))

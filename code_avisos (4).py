...
def load_and_merge_data(uploaded_file_buffer: io.BytesIO) -> pd.DataFrame:
    ...
    df = tmp4[columnas_finales].copy() # Ensure we work on a copy to avoid SettingWithCopyWarning
    ...
    for col in df.columns:
        ...
        for original, normalized in column_mapping.items():
            ...
            if col.strip() == original.strip():
                ...
                found_match = True
                break
            # Then check for case-insensitive and stripped match
            elif col.strip().lower() == original.strip().lower():
                normalized_df_columns.append(normalized)
                found_match = True
                break
        if not found_match:
    ...
    # Ensure 'costes_totreales' is numeric (re-check after potential renaming)
...
def calcular_indicadores(df_filtered_data, group_col='PROVEEDOR'):
    ...
    if not all(col in df_filtered_data.columns for col in required_cols):
        # Removed st.error here as this function is general purpose and should not directly interact with Streamlit
        # However, a more robust error handling or logging could be added.
        print(f"Faltan columnas requeridas para calcular indicadores: {set(required_cols) - set(df_filtered_data.columns)}")
        return (pd.Series(dtype=int), pd.Series(dtype=float), pd.Series(dtype=float),
                pd.Series(dtype=float), pd.Series(dtype=float), pd.Series(dtype=object))
    ...
    mttr = df_filtered_data.groupby(group_col)['TIEMPO PARADA'].mean() # Calculate ttot (total operating time for a service type for each group)
...
class CostosAvisosApp:
    def __init__(self, df):
        ...
        self.opciones_menu = {
            ...
            "Costos por tipo de servicio": ("tipo_de_servicio", self.COL_COSTOS_NORMALIZED, "costos"), # Corrected
            "Avisos por tipo de servicio": ("tipo_de_servicio", None, "avisos"), # Corrected
        }
    ...
    def display_costos_avisos_dashboard(self):
        ...
        with col1:
            selected_year = st.selectbox(
                "Selecciona el Año",
                options=["Todos"] + sorted(self.df['año'].unique().tolist(), reverse=True)
            )
        with col2:
            selected_month = st.selectbox(
                "Selecciona el Mes",
                options=["Todos"] + list(self.df['mes'].unique())
            )
        ...
        group_col, value_col, metric_type = self.opciones_menu[opcion_seleccionada]

        if metric_type == "costos":
            if value_col in df_filtered.columns:
                data_grouped = df_filtered.groupby(group_col)[value_col].sum().sort_values(ascending=False)
                st.write(f"**Top 10 {opcion_seleccionada}**")
                st.dataframe(data_grouped.head(10))

                fig, ax = plt.subplots(figsize=(10, 6))
                data_grouped.head(10).plot(kind='bar', ax=ax, color=sns.color_palette("pastel")[0])
                ax.set_title(f'Top 10 {opcion_seleccionada}')
                ax.set_ylabel('Costos Totales')
                ax.set_xlabel(group_col.replace('_', ' ').title())
                plt.xticks(rotation=45, ha='right')
                st.pyplot(fig)
            else:
                st.warning(f"La columna '{value_col}' no se encontró en los datos para calcular costos.")
        elif metric_type == "avisos":
            data_grouped = df_filtered.groupby(group_col)[self.COL_AVISO_NORMALIZED].nunique().sort_values(ascending=False)
            st.write(f"**Top 10 {opcion_seleccionada}**")
            st.dataframe(data_grouped.head(10))

            fig, ax = plt.subplots(figsize=(10, 6))
            data_grouped.head(10).plot(kind='bar', ax=ax, color=sns.color_palette("pastel")[1])
            ax.set_title(f'Top 10 {opcion_seleccionada}')
        ...
        st.subheader("Análisis de Tiempos de Parada por Ejecutante")
        if 'TIEMPO PARADA' in df_filtered.columns:
            tiempo_parada_por_ejecutante = df_filtered.groupby('PROVEEDOR')['TIEMPO PARADA'].mean().sort_values(ascending=False)
            st.write("**Tiempo de Parada Promedio por Ejecutante (hrs)**")
            st.dataframe(tiempo_parada_por_ejecutante)

            fig, ax = plt.subplots(figsize=(10, 6))
            tiempo_parada_por_ejecutante.head(10).plot(kind='bar', ax=ax, color=sns.color_palette("viridis")[2])
            ax.set_title('Top 10 Proveedores por Tiempo de Parada Promedio')
            ax.set_ylabel('Tiempo de Parada Promedio (hrs)')
            ax.set_xlabel('Proveedor')
            plt.xticks(rotation=45, ha='right')
            st.pyplot(fig)
        else:
            st.warning("La columna 'TIEMPO PARADA' no está disponible para este análisis.")
...
class EvaluacionProveedoresApp:
    ...
    def display_evaluacion_dashboard(self):
        ...
        proveedores = sorted(self.df['PROVEEDOR'].dropna().unique().tolist())
        selected_proveedor = st.selectbox("Selecciona un Proveedor para evaluar:", proveedores)

        if selected_proveedor:
            st.subheader(f"Evaluación para: {selected_proveedor}")

            # Calculate technical performance for the selected provider
            df_proveedor = self.df[self.df['PROVEEDOR'] == selected_proveedor].copy()
            cnt, cost, mttr_val, mtbf_val, disp_val, rend_val = calcular_indicadores(df_proveedor, group_col='PROVEEDOR')

            # Display general information and technical performance
            st.markdown("### Resumen de Desempeño Técnico")
            if not df_proveedor.empty:
                st.write(f"**Número de Avisos:** {cnt.get(selected_proveedor, 0)}")
                st.write(f"**Costo Total:** ${cost.get(selected_proveedor, 0):,.2f}")
                st.write(f"**MTTR (horas):** {mttr_val.get(selected_proveedor, 0):.2f}")
                st.write(f"**MTBF (horas):** {mtbf_val.get(selected_proveedor, 0):.2f}")
                st.write(f"**Disponibilidad (%):** {disp_val.get(selected_proveedor, 0):.2f}%")
                st.write(f"**Rendimiento:** {rend_val.get(selected_proveedor, 'No Aplica')}")
            else:
                st.info("No hay datos técnicos disponibles para este proveedor.")

            st.markdown("### Cuestionario de Evaluación Cualitativa")
            # Initialize scores if not already in session state for this provider
            if f'scores_{selected_proveedor}' not in st.session_state:
                st.session_state[f'scores_{selected_proveedor}'] = {
                    pregunta[1]: 0 for pregunta in self.preguntas if pregunta[2] != "auto"
                }

            scores = st.session_state[f'scores_{selected_proveedor}']
            evaluaciones = []

            for categoria, pregunta_texto, tipo_respuesta in self.preguntas:
                if tipo_respuesta != "auto":
                    st.markdown(f"**Categoría: {categoria}**")
                    st.write(f"**Pregunta:** {pregunta_texto}")
                    opciones_str = tipo_respuesta.split(',')
                    opciones_int = [int(x) for x in opciones_str]
                    opciones_select = []
                    for opt_val in opciones_int:
                        desc = self.rangos_detallados.get(categoria, {}).get(pregunta_texto, {}).get(opt_val, f"Valor {opt_val}")
                        opciones_select.append(f"{opt_val} - {desc}")

                    # Find the current selected index based on the stored score value
                    current_score_val = scores.get(pregunta_texto, 0)
                    try:
                        current_index = opciones_int.index(current_score_val)
                    except ValueError:
                        current_index = opciones_int.index(0) # Default to 0 if not found

                    selected_option_str = st.radio(
                        f"Tu evaluación para: {pregunta_texto}",
                        opciones_select,
                        index=current_index,
                        key=f"{selected_proveedor}_{pregunta_texto}"
                    )
                    # Extract the integer score from the selected string
                    score = int(selected_option_str.split(' ')[0])
                    scores[pregunta_texto] = score
                    evaluaciones.append({'Proveedor': selected_proveedor, 'Categoría': categoria,
                                         'Pregunta': pregunta_texto, 'Puntuación': score})

            st.session_state[f'scores_{selected_proveedor}'] = scores

            df_evaluacion_cualitativa = pd.DataFrame(evaluaciones)
            if not df_evaluacion_cualitativa.empty:
                st.markdown("### Resumen de Puntuaciones Cualitativas")
                st.dataframe(df_evaluacion_cualitativa)

                # Aggregate scores by category
                total_scores_by_category = df_evaluacion_cualitativa.groupby('Categoría')['Puntuación'].sum().reset_index()
                st.subheader("Puntuación Total por Categoría")
                st.dataframe(total_scores_by_category)

                fig, ax = plt.subplots(figsize=(10, 6))
                sns.barplot(x='Categoría', y='Puntuación', data=total_scores_by_category, ax=ax, palette='coolwarm')
                ax.set_title('Puntuación Total por Categoría')
...
if st.session_state['page'] == 'cargar_datos':
    st.header("Cargar Datos de Excel")
    st.write("Por favor, sube tu archivo Excel con las diferentes hojas (IW29, IW39, IH08, IW65, ZPM015).")

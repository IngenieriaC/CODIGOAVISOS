...
class CostosAvisosApp:
    def __init__(self, df):
        ...
        self.opciones_menu = {
            ...
            "Costos por ubicación técnica": ("ubicacion_tecnica", self.COL_COSTOS_NORMALIZED, "costos"),
            "Avisos por ubicación técnica": ("ubicacion_tecnica", None, "avisos"),
            ...
            "Análisis de Costos y Duración por Tipo de Aviso": ("description_category", self.COL_COSTOS_NORMALIZED, "costos_duracion_avisos"),
        }

    def display_costos_avisos_dashboard(self):
        st.header("Dashboard de Costos y Avisos")

        # Selectbox for main category
        main_category = st.selectbox(
            "Selecciona la categoría principal de análisis:",
            list(self.opciones_menu.keys())
        )

        group_col, value_col, analysis_type = self.opciones_menu[main_category]

        if analysis_type == "costos_duracion_avisos":
            self._display_cost_duration_by_notice_type()
        else:
            self._display_general_analysis(group_col, value_col, analysis_type)

    def _display_general_analysis(self, group_col, value_col, analysis_type):
        if group_col not in self.df.columns:
            st.error(f"La columna '{group_col}' no se encontró en los datos.")
            return
        ...
        # Date filter
        df_temp = self.df.copy() # Work on a copy to apply filters
        min_date = df_temp[self.COL_FECHA_AVISO_NORMALIZED].min()
        max_date = df_temp[self.COL_FECHA_AVISO_NORMALIZED].max()

        if pd.isna(min_date) or pd.isna(max_date):
            st.warning("No hay fechas válidas en los datos para aplicar filtros.")
        else:
            ...
            df_temp = df_temp[
                (df_temp[self.COL_FECHA_AVISO_NORMALIZED] >= date_range[0]) &
                (df_temp[self.COL_FECHA_AVISO_NORMALIZED] <= date_range[1])
            ]

        # Dynamic filter for group_col
        unique_values = df_temp[group_col].dropna().unique()
        if unique_values.size > 0:
            selected_group_values = st.multiselect(
                f"Selecciona valores para '{group_col}':",
        ...
        st.subheader(f"Análisis de {main_category}")

        if analysis_type == "costos":
            if value_col not in df_temp.columns:
                st.error(f"La columna '{value_col}' no se encontró en los datos para el análisis de costos.")
                return

            df_summary = df_temp.groupby(group_col)[value_col].sum().sort_values(ascending=False).reset_index()
            df_summary.columns = [group_col, "Total Costos"]

            st.dataframe(df_summary)

            fig, ax = plt.subplots(figsize=(12, 6))
            sns.barplot(x="Total Costos", y=group_col, data=df_summary.head(10), ax=ax, palette='viridis')
            ax.set_title(f'Top 10 {main_category}', fontsize=16)
            ax.set_xlabel('Total Costos', fontsize=12)
            ax.set_ylabel(group_col, fontsize=12)
            st.pyplot(fig)

        elif analysis_type == "avisos":
            df_summary = df_temp.groupby(group_col)[self.COL_AVISO_NORMALIZED].nunique().sort_values(ascending=False).reset_index()
            df_summary.columns = [group_col, "Total Avisos Únicos"]

            st.dataframe(df_summary)

            fig, ax = plt.subplots(figsize=(12, 6))
            sns.barplot(x="Total Avisos Únicos", y=group_col, data=df_summary.head(10), ax=ax, palette='plasma')
            ax.set_title(f'Top 10 {main_category}', fontsize=16)
            ax.set_xlabel('Total Avisos Únicos', fontsize=12)
            ax.set_ylabel(group_col, fontsize=12)
            st.pyplot(fig)

    def _display_cost_duration_by_notice_type(self):
        st.subheader("Análisis de Costos y Duración por Tipo de Aviso")

        df_filtered = self.df.copy()

        # Date filter
        min_date = df_filtered[self.COL_FECHA_AVISO_NORMALIZED].min()
        max_date = df_filtered[self.COL_FECHA_AVISO_NORMALIZED].max()

        if pd.isna(min_date) or pd.isna(max_date):
            st.warning("No hay fechas válidas en los datos para aplicar filtros de fecha.")
            return

        date_range = st.slider(
            "Selecciona el rango de fechas para el análisis de avisos:",
            min_value=min_date.to_pydatetime(),
            max_value=max_date.to_pydatetime(),
            value=(min_date.to_pydatetime(), max_date.to_pydatetime()),
            format="YYYY/MM/DD"
        ...
        df_filtered = df_filtered[
            (df_filtered[self.COL_FECHA_AVISO_NORMALIZED] >= date_range[0]) &
        ...
        # Aggregate data by 'description_category'
        df_analysis = df_filtered.groupby('description_category').agg(
            Total_Costos=('costes_totreales', 'sum'),
            Promedio_Tiempo_Parada=('duracion_de_parada', 'mean'),
            Conteo_Avisos=('aviso', 'nunique')
        ).sort_values(by='Total_Costos', ascending=False).reset_index()

        st.write("### Resumen por Categoría de Aviso")
        st.dataframe(df_analysis)

        # Plotting Total Costs by Description Category
        fig_cost, ax_cost = plt.subplots(figsize=(12, 7))
        sns.barplot(x='Total_Costos', y='description_category', data=df_analysis.head(10), palette='coolwarm', ax=ax_cost)
        ax_cost.set_title('Top 10 Categorías de Aviso por Costos Totales', fontsize=16)
        ax_cost.set_xlabel('Costos Totales', fontsize=12)
        ax_cost.set_ylabel('Categoría de Aviso', fontsize=12)
        st.pyplot(fig_cost)

        # Plotting Average Downtime by Description Category
        fig_time, ax_time = plt.subplots(figsize=(12, 7))
        sns.barplot(x='Promedio_Tiempo_Parada', y='description_category', data=df_analysis.sort_values(by='Promedio_Tiempo_Parada', ascending=False).head(10), palette='magma', ax=ax_time)
        ax_time.set_title('Top 10 Categorías de Aviso por Promedio de Tiempo de Parada', fontsize=16)
        ax_time.set_xlabel('Promedio de Tiempo de Parada (horas)', fontsize=12)
        ax_time.set_ylabel('Categoría de Aviso', fontsize=12)
        st.pyplot(fig_time)
...
class EvaluacionProveedoresApp:
    ...
    def display_evaluacion_dashboard(self):
        st.header("Dashboard de Evaluación de Proveedores")

        # Dropdown para seleccionar el tipo de servicio
        st.subheader("Selección de Tipo de Servicio")
        tipos_servicio_disponibles = self.df['tipo_de_servicio'].dropna().unique()
        if tipos_servicio_disponibles.size == 0:
            st.warning("No hay 'Tipo de Servicio' disponibles en los datos.")
            return

        selected_tipo_servicio = st.selectbox(
            "Selecciona un Tipo de Servicio para evaluar:",
            options=sorted(tipos_servicio_disponibles.tolist())
        )

        df_filtered_by_service = self.df[self.df['tipo_de_servicio'] == selected_tipo_servicio].copy()

        if df_filtered_by_service.empty:
            st.warning(f"No hay datos para el tipo de servicio '{selected_tipo_servicio}'.")
            return

        # Filtrar por proveedores
        st.subheader(f"Evaluación para el Tipo de Servicio: **{selected_tipo_servicio}**")
        proveedores_disponibles = df_filtered_by_service['PROVEEDOR'].dropna().unique()

        if proveedores_disponibles.size == 0:
            st.warning("No hay proveedores disponibles para este tipo de servicio.")
            return

        selected_proveedor = st.selectbox(
            "Selecciona el Proveedor a evaluar:",
            options=sorted(proveedores_disponibles.tolist())
        )

        df_eval_proveedor = df_filtered_by_service[
            df_filtered_by_service['PROVEEDOR'] == selected_proveedor
        ].copy()

        if df_eval_proveedor.empty:
            st.warning(f"No hay datos de avisos para el proveedor '{selected_proveedor}' en este tipo de servicio.")
            return

        st.subheader(f"Preguntas de Evaluación para {selected_proveedor}")
        ...
        # --- Cálculo de indicadores de Desempeño Técnico ---
        # Se calculan los indicadores para el proveedor seleccionado y el tipo de servicio
        cnt_p, cost_p, mttr_p, mtbf_p, disp_p, rend_p = calcular_indicadores(df_eval_proveedor, group_col='PROVEEDOR')
        
        # Asegurarse de que las Series no estén vacías y obtener el valor para el proveedor seleccionado
        current_mttr = mttr_p.get(selected_proveedor, np.nan)
        current_mtbf = mtbf_p.get(selected_proveedor, np.nan)
        current_disp = disp_p.get(selected_proveedor, np.nan)
        current_rend = rend_p.get(selected_proveedor, 'No Aplica')

        # Create a dictionary for quick lookup of technical metrics
        technical_metrics = {
            "MTTR promedio (hrs)": current_mttr,
            "MTBF promedio (hrs)": current_mtbf,
            "Disponibilidad promedio (%)": current_disp,
            "Rendimiento promedio equipos": current_rend
        }

        # Sección para el diligenciamiento de la evaluación manual
        for categoria, pregunta, opciones_str in self.preguntas:
            if categoria == "Desempeño técnico":
                if pregunta in technical_metrics:
                    valor_calculado = technical_metrics[pregunta]
                    st.write(f"**{categoria}**: {pregunta}: **{valor_calculado:.2f}**" if isinstance(valor_calculado, (int, float)) else f"**{categoria}**: {pregunta}: **{valor_calculado}**")
                    
                    # Determine the score based on the calculated value and detailed ranges
                    score = None
                    if pregunta == "Disponibilidad promedio (%)" and not pd.isna(valor_calculado):
                        if valor_calculado >= 98: score = 2
                        elif 75 <= valor_calculado < 98: score = 1
                        else: score = 0
                    elif pregunta == "MTTR promedio (hrs)" and not pd.isna(valor_calculado):
                        if valor_calculado <= 5: score = 2
                        elif 5 < valor_calculado <= 20: score = 1
                        else: score = 0
                    elif pregunta == "MTBF promedio (hrs)" and not pd.isna(valor_calculado):
                        if valor_calculado > 1000: score = 2
                        elif 100 <= valor_calculado <= 1000: score = 1
                        else: score = 0
                    elif pregunta == "Rendimiento promedio equipos":
                        if valor_calculado == 'Alto': score = 2
                        elif valor_calculado == 'Medio': score = 1
                        elif valor_calculado == 'Bajo': score = 0
                        
                    if score is not None:
                        resultados_evaluacion.append({
                            "Categoría": categoria,
                            "Pregunta": pregunta,
                            "Respuesta": valor_calculado,
                            "Puntuación": score
                        })
                        puntuaciones_por_categoria[categoria].append(score)
                continue # Skip manual input for technical questions

            opciones_lista = opciones_str.split(',')
            # Generar opciones para el radio button, incluyendo el texto descriptivo
            radio_options_with_descriptions = []
            for opt_val in opciones_lista:
                try:
                    val_int = int(opt_val)
                    description = self.rangos_detallados[categoria][pregunta][val_int]
                    radio_options_with_descriptions.append(f"{opt_val} - {description}")
                except (ValueError, KeyError):
                    radio_options_with_descriptions.append(opt_val) # Fallback if no description

            respuesta_seleccionada_con_desc = st.radio(
                f"**{categoria}**: {pregunta}",
                radio_options_with_descriptions,
                key=f"{categoria}_{pregunta}"
            )
            
            # Extract the numerical score from the selected radio option
            respuesta_score = int(respuesta_seleccionada_con_desc.split(' ')[0])

            resultados_evaluacion.append({
                "Categoría": categoria,
                "Pregunta": pregunta,
                "Respuesta": respuesta_seleccionada_con_desc,
                "Puntuación": respuesta_score
            })
            puntuaciones_por_categoria[categoria].append(respuesta_score)

        if st.button("Generar Resumen de Evaluación"):
            df_resultados = pd.DataFrame(resultados_evaluacion)
            st.write("### Resultados Detallados de la Evaluación")
            st.dataframe(df_resultados)

            st.write("### Puntuación Promedio por Categoría")
            for categoria, puntuaciones in puntuaciones_por_categoria.items():
                if puntuaciones:
                    promedio = sum(puntuaciones) / len(puntuaciones)
                    st.write(f"- **{categoria}**: {promedio:.2f}")
                else:
                    st.write(f"- **{categoria}**: No aplica (no hay preguntas evaluadas en esta categoría)")

            # Calcular el promedio total de la evaluación
            all_scores = [score for scores in puntuaciones_por_categoria.values() for score in scores]
            if all_scores:
                promedio_total = sum(all_scores) / len(all_scores)
                st.subheader(f"Puntuación Promedio General del Proveedor: **{promedio_total:.2f}**")
            else:
                st.subheader("No hay puntuaciones para calcular el promedio general.")
...
if st.session_state['page'] == 'cargar_datos':
    ...
    st.write("Por favor, sube tu archivo Excel para comenzar el análisis.")

    uploaded_file = st.file_uploader("Sube aquí tu archivo Excel (formatos: .xlsx)", type=["xlsx"])
